...
<body>
	...
	<div class="usershop_login">
		<span class="login_people"><img src="images/6.png" /></span> 
		<a href="hotel/HotelUpdata.jsp">欢迎您*${HOTEL.hotel_name }*</a>
	</div>
	<div class="usershop_indent">
		<a href="HotelSelectIndentServlet">查看所有订单</a>
	</div>
	</header>
	<div class="show_list">
		<div class="meau_left">
			<div class="meau_style">
					<a href="HotelAlterStylemealServlet?type=1");">增加菜系</a>
			</div>
			<c:forEach items="${STYLELIST }" var="stylelist">
				<div class="meau_style">
					<a href="javascript:void(0);"
						onclick="getMeal(${stylelist.style_id });">${stylelist.style_name }</a>
				</div>
			</c:forEach>
			<div class="meau_style">
					<a href="HotelAlterStylemealServlet?type=2");">增加菜品</a>
			</div>
		</div>
		...
		<div class="meau_right" id="inner_html">
			<c:forEach items="${MEALLIST }" var="meallist">
                ...
                -->
					<input type="button" value="移除" code1="${meallist.meal_id }"  code2="${meallist.style_id }" id="remove"/>
				</div>
			</c:forEach>
		</div>
	</div>
		...
		$.ajax({
			...
			success:function(data){
				...
				$.each(data,function(index,item){
					...
					html+='</div>';
					html+='<input type="button" value="移除" code1="'+ item.meal_id +'" id="'+ item.style_id +'"/></div>';
				})
